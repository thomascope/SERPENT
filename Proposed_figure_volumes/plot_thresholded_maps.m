...
%their data range.
clear cfg
cfg.plots = [1:2]; %both hemispheres
% cfg.plots = [1];
...
cfg.rendfile = './Meshes_for_plotting/BrainMesh_ICBM152.gii'; % Different overlay brains

mkdir('./rendered_images/');

all_scans = dir('./tSNR maps/*.nii');
...
for sampling_distance = [3];
    ...
    for i = 1:length(all_scans)
        ...
        jp_spm8_surfacerender2_version_tc(['./tSNR maps/' all_scans(i).name],'parula',cfg)
...
all_scans = dir('./Figure 1 - VBM/*.nii');

for sampling_distance = [3];
    cfg.sampling_distance = sampling_distance; % tc modification - When mesh is lower resolution than overlay image this can be problematic, now displays largest value within a Euclidean distance specified in number of voxels. Can be slow for high resolution images
    for i = 1:length(all_scans)
        
        this_scan_data = spm_read_vols(spm_vol(['./Figure 1 - VBM/' all_scans(i).name]));
         upper_range = max(max(max(this_scan_data)));
%         lower_range = min(min(min(abs(this_scan_data))));
...
lower_range = 0; % data already thresholded
        ...
        jp_spm8_surfacerender2_version_tc(['./Figure 1 - VBM/' all_scans(i).name],'parula',cfg)
