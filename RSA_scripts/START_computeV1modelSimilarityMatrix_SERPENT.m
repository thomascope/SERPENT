function START_computeV1modelSimilarityMatrix_SERPENT(line_drawing_stimuli,photo_stimuli)
...
for image1I=1:nImages
    for i = 1:length(matrices)
        ...
        [S1_im1 ,C1_im1, stim1]=V1modelResponse_SERPENT(squeeze(images(:,:,:,image1I,i)));
        for j = 1:length(matrices)
            this_V1modelSimMat = V1modelSimMat;
            parfor image2I=1:nImages
                
                % load images and compute V1 represenation
                [S1_im2 ,C1_im2, stim2]=V1modelResponse_SERPENT(squeeze(images(:,:,:,image2I,j)));
            ...
            V1RDM(i,j).RDM(image1I,:) = this_V1modelSimMat(image1I,1:nImages);
            V1RDM(i,j).name =  [matrices{i} ' to ' matrices{j} ' V1 model dissimilarity'];
            V1RDM(i,j).shortname =  [matrices{i} ' to ' matrices{j}];
            
        end
    end
end % image1I
save(fullfile(resultsPath,'V1RDM'),'V1RDM');
