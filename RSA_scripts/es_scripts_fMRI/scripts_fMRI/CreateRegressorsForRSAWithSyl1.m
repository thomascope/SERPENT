...
for sess=1:NumSess
    ...
    for itemset=1:8
        for i=1:4
            counter = counter + 1;
            conIdx{counter} = find( ismember(tmp{:,21},'clearSpeech')==1 & tmp{:,18}==2 & tmp{:,20}==1 & tmp{:,17}==itemset & tmp{:,19}==i & tmp{:,5}==0 ); % Non-targets
            names{counter} = sprintf('Strong+M_Set%d_Item%d',itemset,i);
    ...
    for itemset=1:8
        for i=1:4
            counter = counter + 1;
            conIdx{counter} = find( ismember(tmp{:,21},'clearSpeech')==1 & tmp{:,18}==1 & tmp{:,20}==1 & tmp{:,17}==itemset & tmp{:,19}==i & tmp{:,5}==0 ); % Non-targets
            names{counter} = sprintf('Weak+M_Set%d_Item%d',itemset,i);
            onsets{counter} = double(tmp{6}(conIdx{counter}));
            durations{counter} = zeros(length(onsets{counter}),1);
        end
    end
    for itemset=1:8
        for i=1:4
            counter = counter + 1;
            conIdx{counter} = find( ismember(tmp{:,21},'clearSpeech')==1 & tmp{:,18}==2 & tmp{:,20}==2 & tmp{:,17}==itemset & tmp{:,19}==i & tmp{:,5}==0 ); % Non-targets
            names{counter} = sprintf('Strong+MM_Set%d_Item%d',itemset,i);
            onsets{counter} = double(tmp{6}(conIdx{counter}));
            durations{counter} = zeros(length(onsets{counter}),1);
        end
    end
    for itemset=1:8
        for i=1:4
            counter = counter + 1;
            conIdx{counter} = find( ismember(tmp{:,21},'clearSpeech')==1 & tmp{:,18}==1 & tmp{:,20}==2 & tmp{:,17}==itemset & tmp{:,19}==i & tmp{:,5}==0 ); % Non-targets
            names{counter} = sprintf('Weak+MM_Set%d_Item%d',itemset,i);
            onsets{counter} = double(tmp{6}(conIdx{counter}));
            durations{counter} = zeros(length(onsets{counter}),1);
        end
    end
...
%         for i=1:4
%             counter = counter + 1;
%             conIdx{counter} = find( ismember(tmp{:,21},'noiseFinal')==1 & tmp{:,18}==2 & tmp{:,17}==itemset & tmp{:,19}==i & tmp{:,5}==0 ); % Non-targets
%             names{counter} = sprintf('Strong+Noise_Set%d_Item%d',itemset,i);
%             onsets{counter} = double(tmp{6}(conIdx{counter}));
%             durations{counter} = zeros(length(onsets{counter}),1);
%         end
%     end
...
%         for i=1:4
%             counter = counter + 1;
%             conIdx{counter} = find( ismember(tmp{:,21},'noiseFinal')==1 & tmp{:,18}==1 & tmp{:,17}==itemset & tmp{:,19}==i & tmp{:,5}==0 ); % Non-targets
%             names{counter} = sprintf('Weak+Noise_Set%d_Item%d',itemset,i);
%             onsets{counter} = double(tmp{6}(conIdx{counter}));
%             durations{counter} = zeros(length(onsets{counter}),1);
%         end
%     end
    for itemset=1:8
        for i=1:4
            counter = counter + 1;
            conIdx{counter} = find( ismember(tmp{:,21},'noiseInitial')==1 & tmp{:,17}==itemset & tmp{:,19}==i & tmp{:,5}==0 ); % Non-targets
            names{counter} = sprintf('Noise+Speech_Set%d_Item%d',itemset,i);
            onsets{counter} = double(tmp{6}(conIdx{counter}));
            durations{counter} = zeros(length(onsets{counter}),1);
        end
    end
