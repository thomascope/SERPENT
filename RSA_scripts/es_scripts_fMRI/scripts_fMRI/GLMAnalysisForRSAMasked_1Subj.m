...
%matlabbatch{1}.spm.stats.fmri_spec.mask = {[ProcDataDir '/ROIs/wLeft_STG_ManovaSyl2.nii']};
StrDir = fullfile(ProcDataDir,'Structural');
GreyMatterImage = cellstr(spm_select('FPList', StrDir, '^c1.*\.nii$'));
V = spm_vol(GreyMatterImage{1});
mask = spm_read_vols(V);
mask(mask<=.9) = NaN;
V.fname = strrep(GreyMatterImage{1},'.nii','_p09.nii');
spm_write_vol(V,mask);
matlabbatch{1}.spm.stats.fmri_spec.mask = {V.fname};
