...
Tcount = Tcount+1;
contrastTmp = zeros(1,length(names));
ind = find(ismember(names,'Strong+M'));
contrastTmp(ind) = 1/length(ind);
Tcons(Tcount).c = contrastTmp;
Tcons(Tcount).name = 'Strong+M';

Tcount = Tcount+1;
contrastTmp = zeros(1,length(names));
ind = find(ismember(names,'Weak+M'));
contrastTmp(ind) = 1/length(ind);
Tcons(Tcount).c = contrastTmp;
Tcons(Tcount).name = 'Weak+M';

Tcount = Tcount+1;
contrastTmp = zeros(1,length(names));
ind = find(ismember(names,'Strong+MM'));
contrastTmp(ind) = 1/length(ind);
Tcons(Tcount).c = contrastTmp;
Tcons(Tcount).name = 'Strong+MM';

Tcount = Tcount+1;
contrastTmp = zeros(1,length(names));
ind = find(ismember(names,'Weak+MM'));
contrastTmp(ind) = 1/length(ind);
Tcons(Tcount).c = contrastTmp;
Tcons(Tcount).name = 'Weak+MM';

Tcount = Tcount+1;
contrastTmp = zeros(1,length(names));
ind = find(ismember(names,'Strong+Noise'));
contrastTmp(ind) = 1/length(ind);
Tcons(Tcount).c = contrastTmp;
Tcons(Tcount).name = 'Strong+Noise';

Tcount = Tcount+1;
contrastTmp = zeros(1,length(names));
ind = find(ismember(names,'Weak+Noise'));
contrastTmp(ind) = 1/length(ind);
Tcons(Tcount).c = contrastTmp;
Tcons(Tcount).name = 'Weak+Noise';

Tcount = Tcount+1;
contrastTmp = zeros(1,length(names));
ind = find(ismember(names,'Noise+Speech'));
contrastTmp(ind) = 1/length(ind);
Tcons(Tcount).c = contrastTmp;
Tcons(Tcount).name = 'Noise+Speech';

Tcount = Tcount+1;
contrastTmp = zeros(1,length(names));
ind = find(ismember(names,'Noise+Noise'));
contrastTmp(ind) = 1/length(ind);
Tcons(Tcount).c = contrastTmp;
Tcons(Tcount).name = 'Noise+Noise';

Tcount = Tcount+1;
contrastTmp = zeros(1,length(names));
ind = [find(ismember(names,'Strong+M')) find(ismember(names,'Strong+MM'))];
contrastTmp(ind) = 1/length(ind);
Tcons(Tcount).c = contrastTmp;
Tcons(Tcount).name = 'Strong+Clear';

Tcount = Tcount+1;
contrastTmp = zeros(1,length(names));
ind = [find(ismember(names,'Weak+M')) find(ismember(names,'Weak+MM'))];
contrastTmp(ind) = 1/length(ind);
Tcons(Tcount).c = contrastTmp;
Tcons(Tcount).name = 'Weak+Clear';

% Selected contrasts
Tcount = Tcount+1;
contrastTmp = zeros(1,length(names));
ind = [find(ismember(names,'Strong+M')) find(ismember(names,'Strong+MM')) find(ismember(names,'Weak+M')) find(ismember(names,'Weak+MM'))];
contrastTmp(ind) = 1/length(ind);
ind = find(ismember(names,'Noise+Noise'));
contrastTmp(ind) = -1/length(ind);
Tcons(Tcount).c = contrastTmp;
Tcons(Tcount).name = 'Clear > Noise+Noise';

Tcount = Tcount+1;
Tcons(Tcount).c = contrastTmp.*-1;
Tcons(Tcount).name = 'Noise+Noise > Clear';

Tcount = Tcount+1;
contrastTmp = zeros(1,length(names));
ind = find(ismember(names,'HitsAndFalseAlarms'));
contrastTmp(ind) = 1/length(ind);
Tcons(Tcount).c = contrastTmp;
Tcons(Tcount).name = 'HitsAndFalseAlarms';

% Tcount = Tcount+1;
% contrastTmp = zeros(1,length(names));
% ind = find(ismember(names,'Misses'));
% contrastTmp(ind) = 1/length(ind);
% Tcons(Tcount).c = contrastTmp;
% Tcons(Tcount).name = 'Misses';
