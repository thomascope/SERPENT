...
%		subjectMetadataStruct = spawnSPMStruct;
		...
		for maskNumber = 1:nMasks % For each mask...
			...
			for modelNumber = 1:numel(models)
				...
				%modelName = spacesToUnderscores(models(modelNumber).name); % es edit- missing this function
...
function [smm_rs, smm_ps, n, searchlightRDMs] = searchlightMapping_fMRI(fullBrainVolumes, models, mask, userOptions, localOptions)
	...
	%searchlightRDMs = nan([nConditions, nConditions, volSize_vox]); % es edit, takes too much memory
	...
	for cMappingVoxI=1:nVox_mappingMask_request
		...
		%searchlightRDMs(:, :, x, y, z) = squareform(searchlightRDM); % es edit, takes too much memory
        searchlightRDMs(:, :, 1, 1, 1) = squareform(searchlightRDM); % es edit
...
%         rs = searchlightRDM*modelRDMs_ltv';  % es edit
%         ps = rs;
