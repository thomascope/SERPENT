function module_make_partial_effect_maps_focussed(GLMDir,downsamp_ratio,subject,matrices_to_partial)
...
basemodels.patient_judgment_actually_noself = overall_patient_judgment_matrix;
...
load('/group/language/data/thomascope/7T_SERPENT_pilot_analysis/V1model_results/V1RDM_ds.mat')
...
for j = 1:size(cross_decode_label_pairs,1)
    basemodels.V1_ds_pp_noself = V1RDM_ds(strcmp(cross_decode_label_pairs{j,1},visual_matrices),strcmp(cross_decode_label_pairs{j,2},visual_matrices)).RDM;
    basemodels.V1_ds_pp_noself(1:16:end) = NaN;
end
basemodelNames(end+1) = {'V1_ds_pp_noself'};
...
model_unsorted = modeltemplate;
for j = 1:length(all_combinations)
    model_unsorted(15*(j-1)+1:15*j,15*(j-1)+1:15*j) = V1RDM_ds(j,j).RDM;
end
models{end+1} = model_unsorted(joined_table_originalorder.designnumber,joined_table_originalorder.designnumber);
this_model_name{end+1} = ['All V1_ds'];
...
models{end+1} = model_unsorted(joined_table_originalorder.designnumber,joined_table_originalorder.designnumber);
this_model_name{end+1} = ['Photo to Line V1_ds'];
...
models{end+1} = model_unsorted(joined_table_originalorder.designnumber,joined_table_originalorder.designnumber);
this_model_name{end+1} = ['Global GIST correlation'];
...
models{end+1} = model_unsorted(joined_table_originalorder.designnumber,joined_table_originalorder.designnumber);
this_model_name{end+1} = ['Global V1_ds'];
...
parfor m=1:length(this_model_name) %Parallelising here impossible due to out of memory on serialisation unless data downsampled
    ...
    for these_partial_numbers = 1:length(partial_matrices)
        ...
        for this_partial = 1:size(all_partial_combinations,1)
            ...
            if ~exist(fullfile(outputDir,['effect-map_' this_model_name{m} partial_name '.nii']),'file') || redo_maps == 1
                modelRDM = vectorizeRDMs(models{m})';
                partialRDMs = [];
                for this_partial_matrix = 1:number_partialled_out
                    partialRDMs = [partialRDMs vectorizeRDMs(models{all_partial_combinations(this_partial,this_partial_matrix)})'];
                end
                ...
                for vx=1:numel(data)
                    ...
                    if ~isempty(strfind(version,'pearson'))
                        effectMap(mask_index(vx)) = fisherTransform(partialcorr(modelRDM,neuralRDM,partialRDMs,'type','Pearson','Rows','pairwise'));
                    elseif ~isempty(strfind(version,'spearman'))
                        effectMap(mask_index(vx)) = fisherTransform(partialcorr(modelRDM,neuralRDM,partialRDMs,'type','Spearman','Rows','pairwise'));
                    end
                    if ~mod(vx,100)
                ...
                saveMRImage(downsamped_effectMap,fullfile(outputDir,['effect-map_' this_model_name{m} partial_name '.nii']),downsamped_V.mat);
            else
