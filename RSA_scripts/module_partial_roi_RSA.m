function module_partial_roi_RSA(GLMDir,mask_names,subject,matrices_to_partial)
    ...
    for m=1:length(this_model_name) %Parallelising here impossible due to out of memory on serialisation unless data downsampled
        ...
        for these_partial_numbers = 1:length(partial_matrices)
            ...
            for this_partial = 1:size(all_partial_combinations,1)
                ...
                if ~exist(fullfile(outputDir,['roi_effects_' this_model_name{m} partial_name '.mat'])) || redo_maps == 1
                    ...
                    for vx=1:numel(data)
                        ...
                        if ~isempty(strfind(version,'pearson'))
                            roi_effect(vx) = fisherTransform(partialcorr(modelRDM,neuralRDM,partialRDMs,'type','Pearson','Rows','pairwise'));
                        elseif ~isempty(strfind(version,'spearman'))
                            roi_effect(vx)  = fisherTransform(partialcorr(modelRDM,neuralRDM,partialRDMs,'type','Spearman','Rows','pairwise'));
                        end
                    end
                    save(fullfile(outputDir,['roi_effects_' this_model_name{m} partial_name '.mat']),'roi_effect','roi_names')
                else
