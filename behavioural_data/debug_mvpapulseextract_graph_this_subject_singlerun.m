function [ all_response_averages, all_rt_averages] = debug_mvpapulseextract_graph_this_subject_singlerun( subject, runnum, date )

%all_files = ls(['*' subject '*' num2str(date) '.mat']);
all_files = ['PNFA_7T_' subject '_Run_' num2str(runnum) '_' num2str(date) '.mat'];

[startpulses,stimType,stimNumber,stimName] = extract_pulsenumbers_from_second_fullparadigm(all_files,runnum);
