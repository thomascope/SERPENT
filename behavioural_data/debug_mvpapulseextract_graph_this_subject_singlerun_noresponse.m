function [ all_response_averages, all_rt_averages] = debug_mvpapulseextract_graph_this_subject_singlerun_noresponse( subject, runnum, date )

%all_files = ls(['*' subject '*' num2str(date) '.mat']);
all_files = ['PNFA_7T_' subject '_Run_' num2str(runnum) '_' num2str(date) '.mat'];

[startpulses,stimType,stimNumber,stimName] = extract_pulsenumbers_from_second_fullparadigm_noresponse(all_files,runnum);
...
%Now check we are able to reconstruct from the pulse numbers

all_response_averages = [nanmean(nanpadded_runs_resps(mod(stimType(1:108),2)&floor((stimType(1:108)+1)/2)==1)) nanmean(nanpadded_runs_resps(mod(stimType(1:108),2)&floor((stimType(1:108)+1)/2)==2)) nanmean(nanpadded_runs_resps(mod(stimType(1:108),2)&floor((stimType(1:108)+1)/2)==3)) nanmean(nanpadded_runs_resps(~mod(stimType(1:108),2)&floor((stimType(1:108)+1)/2)==1)) nanmean(nanpadded_runs_resps(~mod(stimType(1:108),2)&floor((stimType(1:108)+1)/2)==2)) nanmean(nanpadded_runs_resps(~mod(stimType(1:108),2)&floor((stimType(1:108)+1)/2)==3))]
figure
bar(all_response_averages)
ylim([1 4])
title('Clarity Rating')
