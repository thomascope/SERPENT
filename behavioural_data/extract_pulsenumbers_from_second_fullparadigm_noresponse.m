function [startpulses,stimType,stimNumber,stimName] = extract_pulsenumbers_from_second_fullparadigm_noresponse(fileName,runI)
...
startpulses = [imputed_pulse_numbers_at_normal_trialstart, imputed_pulse_numbers_at_writtenonly_trialstart];
...
for i = 1:length(written_index)
    ...
    elseif strcmp('Neutral',cue_types{response_order.this_cue_types(j)})
        stimNumber(end+1) = response_order.this_mismatch_cue(j);
    elseif strcmp('Match',cue_types{response_order.this_cue_types(j)})
        stimNumber(end+1) = 0;
    end
end
