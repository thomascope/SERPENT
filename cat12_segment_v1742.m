function cat12_segment_v1742(ref)
...
%Try to force this output
cat_get_defaults('output.bias.native',1)
cat_get_defaults('output.GM.warped',1)
cat_get_defaults('output.WM.warped',1)
cat_get_defaults('opts.biasstr',1.0); % Use 1 for inisital mask creation from MP2RAGE
%def.opts.biasfwhm def.opts.biasstr
disp(['Reference images is ' ref '.']);
...
%-----------------------------------------------------------------------
matlabbatch{1}.spm.tools.cat.estwrite.data = {ref};
matlabbatch{1}.spm.tools.cat.estwrite.data_wmh = {''};
matlabbatch{1}.spm.tools.cat.estwrite.nproc = 0;  %Cores to use. 8 = 8, 0 = 1?
matlabbatch{1}.spm.tools.cat.estwrite.useprior = '';
matlabbatch{1}.spm.tools.cat.estwrite.opts.tpm = {'/rds/project/rds-k1PgZFWfeZY/applications/spm/spm12_7771/tpm/TPM.nii'};
matlabbatch{1}.spm.tools.cat.estwrite.opts.affreg = 'mni';
matlabbatch{1}.spm.tools.cat.estwrite.opts.biasstr = 0.5;  %Change to 1 for maximum bias adjustment. eps for ultralight. EXPERT OPTIONS ARE IGNORED HERE. MUST CHANGE through cat_get_defaults above
...
matlabbatch{1}.spm.tools.cat.estwrite.extopts.registration.shooting.shootingtpm = {'/rds/project/rds-k1PgZFWfeZY/applications/spm/spm12_7771/toolbox/cat12/templates_volumes/Template_0_IXI555_MNI152_GS.nii'};
