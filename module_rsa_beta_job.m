function [avgRDM, stats_p_r] = module_rsa_beta_job(betapattern_numbers,mask_path,data_path,condition_number,condition_name,num_sess)
...
for sessionI=1:num_sess
    ...
    for pat = 1:size(betapattern_numbers,2)
        try
            raw_pattern = spm_read_vols(spm_vol([data_path 'Cbeta_denan_' sprintf('%04d',betapattern_numbers(sessionI,pat)) '.nii']));
        catch
            %module_fslmaths_job([data_path 'Cbeta_' sprintf('%04d',betapattern_numbers(sessionI,pat)) '.nii'],'-nan',[data_path 'Cbeta_denan_' sprintf('%04d',betapattern_numbers(sessionI,pat)) '.nii']); %Account for the fact that spm_read_vols crashes with nan
            try
                raw_pattern = spm_read_vols(spm_vol([data_path 'Cbeta_denan_' sprintf('%04d',betapattern_numbers(sessionI,pat)) '.nii']));
            catch
                error(['failed at ' data_path 'Cbeta_denan_' sprintf('%04d',betapattern_numbers(sessionI,pat)) '.nii'])
            end
        end
        if min(size(mask) == size(raw_pattern)) ~= 1
            error('Your mask and beta-patterns are not in the same space, try again')
        end
...
avgRDM.name=sprintf('RDM across sessions | condition %s',condition_name);
...
nobjects = length(betapattern_numbers);
...
options.categoryColors=zeros(length(betapattern_numbers),3);
