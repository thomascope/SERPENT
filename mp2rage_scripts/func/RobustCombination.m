...
if isfield(MP2RAGE,'filenameOUT')
    if ~isempty(MP2RAGE.filenameOUT)
        if integerformat==0
            ...
            mult=4095; %VALUE to scale all interger values by. Could use 32767 to maxmise the range or 4095 to make it like Siemens original
             MP2RAGEimg.img=round(mult*(MP2RAGEimgRobustPhaseSensitive+0.5)/max(MP2RAGEimgRobustPhaseSensitive(:)+0.5)); %PSJ
%            MP2RAGEimg.img=round(4095*(MP2RAGEimgRobustPhaseSensitive+0.5)); %ORIGINAL
            save_untouch_nii(MP2RAGEimg,MP2RAGE.filenameOUT);
