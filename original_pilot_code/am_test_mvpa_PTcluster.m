...
stimdelay    = 1.70; % the audio stimulus was presented 2200ms after the scanner pulse
...
if ~(exist(['tpatterns_masked_PT_' subj_initials '.mat'],'file'))
...
if ~(exist(['tpatterns_masked_PT_' subj_initials '.mat'],'file'))
    % save the t-patterns
    save(['tpatterns_masked_PT_' subj_initials '.mat'], 'tpatterns','wholebrain_tpatterns')
else
    % if the t-patterns already exist --> load it
    %load('tpatterns.mat')
    load(['tpatterns_masked_PT_' subj_initials '.mat'])
end
