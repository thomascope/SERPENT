...
mask = logical(spm_read_vols(spm_vol('Left_PT_Cluster.nii')));
...
for runI=1:nRuns
    ...
    this_file = dir(['P*' subj_initials '_Run_' num2str(runI) '_' testing_date '.mat']);
...
for runI=1:nRuns
    ...
    for condI=1:length(tempOnsets)
        design{runI}(round((tempDesign{runI}{condI}-stimdelay)/tr)+2,condI) = 1;
    end
...
if ~(exist(['tpatterns_masked_PT_T2_' subj_initials '.mat'],'file'))
...
if ~(exist(['tpatterns_masked_PT_T2_' subj_initials '.mat'],'file'))
    % save the t-patterns
    save(['tpatterns_masked_PT_T2_' subj_initials '.mat'], 'tpatterns','wholebrain_tpatterns')
else
    % if the t-patterns already exist --> load it
    %load('tpatterns.mat')
    load(['tpatterns_masked_PT_T2_' subj_initials '.mat'])
end
