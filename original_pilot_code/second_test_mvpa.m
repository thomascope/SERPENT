...
%clear all
[workingdir,file,ext] = fileparts(which('second_test_mvpa.m'));
...
nVolumes     = 200; % there were at least 200 EPI volumes acquired per run
tr           = 3;   % the EPI volumes were acquired every 3 seconds
...
structural_fname = 'skullstripped_WRBJ.nii';
wholespace_mask = 'wholespace_mask.nii';
%%% NEED A BETTER MASK
%mask = logical(spm_read_vols(spm_vol('Univariate_Mask.nii')));
mask = logical(spm_read_vols(spm_vol('Univariate_cluster_mask.nii')));
...
if ~(exist('tpatterns_masked_2.mat','file'))
    %Create whole brain masked input files
    
    if isempty(dir(['ma8*' subj_initials '_Run_1_' testing_date '.nii']));
        input_files = dir(['a8*' subj_initials '_Run_*_' testing_date '.nii']);
        cbupool(nRuns)
        parfor i = 1:nRuns
            spm_mask(structural_fname, input_files(i).name)
        end
        matlabpool close
    end
    ...
    for session = 1:2
        data = cell(size(session:2:nRuns));
        % load the nifti data;
        for runI=1:(nRuns/2) %XXX Relies on an even number of runs at present
            fprintf('***\t importing EPI time-series for run %d \t***\n',session+(2*(runI-1)))
            this_file = dir(['ma*' subj_initials '_Run_' num2str(session+(2*(runI-1))) '_' testing_date '.nii']);
            fileName = this_file.name;
        ...
        results = GLMdenoisedata(design(session:2:nRuns),data,stimdur,tr, ...
            'optimize',[],struct('numboots',50,'numpcstotry',10,'wantparametric',1), ...
...
if ~(exist('tpatterns_masked_2.mat','file'))
    % save the t-patterns
    save('tpatterns_masked_2.mat', 'tpatterns','wholebrain_tpatterns')
else
    % if the t-patterns already exist --> load it
    %load('tpatterns.mat')
    load('tpatterns_masked_2.mat')
end
...
if analyse.searchlight
    ...
    addpath('/imaging/mlr/users/tc02/7T_full_paradigm_pilot/Preprocessed_Images')
    for condJ = 1:nTrialtypes
        %Define matrix based on shared features
        ...
        userOptions.betaPath = '/imaging/mlr/users/tc02/7T_full_paradigm_pilot/Preprocessed_Images/WRBJ_univariate/beta_[[betaIdentifier]].nii';
        %userOptions.subjectNames = {subj_initials};
        userOptions.analysisName = ['Condition_' num2str(condJ)];
        userOptions.rootPath = '/imaging/mlr/users/tc02/7T_full_paradigm_pilot/Preprocessed_Images/Searchlight';
