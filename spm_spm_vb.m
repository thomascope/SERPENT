...
%-----------------------------------------------------------------------
mkdir('temp_estimate')
save('temp_estimate/pre_estimate_workspace.mat')
% 
% try
%     cbupool(ceil(nLb/2))
% catch
    ...
    module_parallel_VB(z);
        ...
        module_parallel_VB(z);
            ...
            module_parallel_VB(z);
                ...
                error(['failed for block ' num2str(z)])
            end
        end
    end
...
for z = 1:nLb
    temp_PPM = load(['temp_estimate/SPM_PPM_' num2str(z) '.mat']);
    for s = 1:nsess
        SPM.PPM.Sess(s).block(z) = temp_PPM.SPM.PPM.Sess(s).block(z);
    end
